...
- [ ] **analyze phase** — pre-implementation alignment gate (spec×plan×tasks consistency + plan vs codebase reality check). Independent phase, hard gate before implement. [done: SKILL.md + pipeline integration]
...
### v4+ — Exploration
- [ ] **audit phase** — pipeline final station combining release + prune-review + reflection into one final verification step before archive (non-blocking, advisory). Merges changelog, complexity check, learning record, and reference update. Requires v3.1 analyze pattern before detailed design.
