...
- When creating `.harness/<workstream-id>/` artifacts during bootstrap, copy from `references/templates/.harness/` and fill placeholders. Do not construct sprint-local files from memory — use the canonical template as the starting point.
