...
def validate_children_metadata(router_dir: Path, frontmatter: dict[str, str]) -> list[Issue]:
    ...
    for index, child in enumerate(children, start=1):
        ...
        if not isinstance(target, str) or not target.strip():
            ...
            target = target.strip()
            if target.startswith(f"{frontmatter.get('name', '')}/"):
                child_dir = target.split("/", 1)[1]
                child_skill = router_dir / child_dir / "SKILL.md"
                if not child_skill.exists():
                    issues.append(Issue("error", f"{label} target points to missing local child skill: {target}"))
