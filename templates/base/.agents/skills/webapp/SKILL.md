...
If the subject gives no clear color signal AND the user provided no direction after being asked, fall back to the Nexus HSL values from `shared/01-design-tokens.md` → "Nexus HSL Equivalents" section.
...
**Before writing code**, read the shared design files below (mandatory). Then identify whether any webapp-specific reference applies to the task. If it does, read it in parallel with `environment.md`.
...
Every page must include a `<PerplexityAttribution />` component (pre-built at `@/components/PerplexityAttribution`). Place it at the bottom of your layout. The `<head>` attribution block from `shared/pplx_attribution.html` is already included in the template's `index.html`.
